...
invcounts_target = pd.read_sql_table('inventory_invcount', target_engine)
...
bk_joe = invcounts_source.groupby(['fac', 'period', 'imms']).agg({'qty': ['sum']})

# pivot quantity out on facility, period, and imms #
piv = pd.pivot_table(invcounts_source, values='qty', index=['fac', 'period', 'imms'],
    aggfunc=np.sum)
...
i2020 = issues2020.groupby(['fac', 'imms'], as_index=False)['qty'].sum()

# merge grouped issues on grouped counts
invcount_nov_issues = most_recent_count.merge(i2020, how="left", on=["fac", "imms"])
...
# group the issues dataframe by imms and facility and sum the qty 
p2020 = po2020.groupby(['fac', 'imms'], as_index=False)['poQty'].sum()

# merge grouped issues on grouped counts
invcount_joined = invcount_nov_issues.merge(p2020, how="left", on=["fac", "imms"])
invcount_joined = invcount_joined.fillna(0)
invcount_joined = invcount_joined.rename(columns={'poQty': 'po_qty'})

invcount_joined.count_qty = invcount_joined.count_qty.astype('int')
invcount_joined.po_qty = invcount_joined.po_qty.astype('int')
...
cleaned_items = items[['fac', 'TenetFacilityName', 'imms', 'Mfr', 'MfrCat', 'ItemDesc', 
                       ...
                       'DefaultUOMPrice', 'UOM1', 'CONV1']]

cleaned_items = cleaned_items.rename(columns={'TenetFacilityName': 'facility_name', 'Mfr': 'mfr', 'MfrCat': 'mfr_cat_no', 'ItemDesc': 'description',
                                             'MMISItemCreateDate': 'imms_create_date', 'Vend': 'vendor', 'VendCat': 'vend_cat_no', 'DefaultUOM': 'default_uom',
                                             'DefaultUOMConv': 'default_uom_conv', 'DefaultUOMPrice': 'default_uom_price', 'UOM1': 'luom', 'CONV1': 'luom_conv'})
...
cleaned_items = cleaned_items.sort_values(by='default_uom_conv', ascending=True, ignore_index=True)
...
final = invcount_joined.merge(new_items, how="left", on=['fac', 'imms'])

#drop items that item master could not pick up
final = final.dropna().reset_index(drop=True)

# change UOM conv's to integers
final.default_uom_conv = final.default_uom_conv.astype('int')
final.luom_conv = final.luom_conv.astype('int')
